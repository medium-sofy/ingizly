...
<div class="p-10 bg-gray-100 min-h-screen">
    <div class="max-w-7xl mx-auto">
        <div class="mb-8 text-center">
            <h1 class="text-4xl font-extrabold text-gray-800">Wallet Overview</h1>
            <p class="text-gray-500 mt-2 text-lg">Manage your payments and download transaction receipts easily</p>
        </div>
                ...
                @forelse($payments as $payment)
                    <div class="bg-gray-50 shadow rounded-xl p-4">
                        <div class="flex justify-between items-center mb-2">
                            <div class="text-sm font-semibold text-gray-700">Order #{{ $payment->order->id }}</div>
                        ...
                        <div class="text-gray-600 text-sm mb-1"><strong>Buyer:</strong> {{ $payment->order->user->name }}</div>
                        <div class="text-gray-600 text-sm mb-1"><strong>Amount:</strong> {{ $payment->amount }} {{ $payment->currency }}</div>
                        <div class="text-gray-600 text-sm"><strong>Transaction ID:</strong> {{ $payment->transaction_id }}</div>
                    </div>
                @empty
